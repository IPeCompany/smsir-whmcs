<?php
use Illuminate\Database\Capsule\Manager as Capsule;
use WHMCS\Module\Addon\smsir\vahabonline\vahab;
use WHMCS\Module\Addon\smsir\vahabonline\hdata;

function smsir_vo_ClientAdd($params){
    $hookname = 'ClientAdd';
    $uid = $params['client_id'];
    $replaces = [];
    vahab::SendMessageForHook($hookname, $replaces, $uid);
}



function smsir_vo_ClientAdd_install($hookname){
    try{
        Capsule::table('smsir_vo_hooks')->insert([
            'name' => $hookname,
            'label' => 'هنگام عضویت کاربر',
            'params' => vahab::ShowUserDefaultTag(),
            'admin_params' => vahab::ShowUserDefaultTag(),
            'type_send' => 'default',
            'send_for' => 'all',
            'message' => "{firstname} عزیز \nاز عضویت شما در سامانه بسیار خرسندیم . امیدواریم بتوانیم خدمات مناسب و در خورد خدمت شما بزرگوار ارائه نماییم \n{signature}",
            'admin_message' => "{firstname} عزیز \nاز عضویت شما در سامانه بسیار خرسندیم . امیدواریم بتوانیم خدمات مناسب و در خورد خدمت شما بزرگوار ارائه نماییم \n{signature}",
            'status' => 1,
            'user_status' => 0,
            'admin_status' => 0
        ]);
    }catch(Exception $e){}
}