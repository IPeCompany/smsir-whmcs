...
function smsir_vo_AfterRegistrarRegistrationFailed($params){
    $hookname = 'AfterRegistrarRegistrationFailed';
...
function smsir_vo_AfterRegistrarRegistrationFailed_install($hookname){
    try{
        Capsule::table('smsir_vo_hooks')->insert([
            'name' => $hookname,
            'label' => 'خطا در ثبت دامنه توسط رجیسترارر',
            ...
            'send_for' => 'admin',
            'message' => "",
            'admin_message' => "{firstname} عزیز \nدامنه {sld}.{tld} هنگام ثبت با خطا مواجه شد \n{signature}",
            'status' => 1,
            'user_status' => 0,
            'admin_status' => 0
        ]);
