...
function smsir_vo_AddonCancelled($params){
    $hookname = 'AddonCancelled';
    $id = $params['id'];
    $uid = $params['uid'];
...
function smsir_vo_AddonCancelled_install($hookname){
    try{
        Capsule::table('smsir_vo_hooks')->insert([
            'name' => $hookname,
            'label' => 'کنسل شدن افزودنی',
