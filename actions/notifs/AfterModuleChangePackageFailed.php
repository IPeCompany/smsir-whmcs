...
function smsir_vo_AfterModuleChangePackageFailed($params){
    $hookname = 'AfterModuleChangePackageFailed';
...
function smsir_vo_AfterModuleChangePackageFailed_install($hookname){
    try{
        Capsule::table('smsir_vo_hooks')->insert([
            'name' => $hookname,
            'label' => 'خطا در تغییر پکیج',
            ...
            'type_send' => 'default',
            'send_for' => 'all',
            'message' => "{firstname} عزیز \nتغییر پکیج سرویس {domain} با خطا مواجه شد \n{signature}",
