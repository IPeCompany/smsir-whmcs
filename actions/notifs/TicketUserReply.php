...
function smsir_vo_TicketUserReply($params){
    $hookname = 'TicketUserReply';
...
function smsir_vo_TicketUserReply_install($hookname){
    try{
        Capsule::table('smsir_vo_hooks')->insert([
            'name' => $hookname,
            'label' => 'پاسخ به تیکت توسط کاربر',
            ...
            'type_send' => 'default',
            'send_for' => 'all',
            'message' => "{firstname} عزیز \nپاسخ جدیدی به تیکت {tid} با موضوع {subject} ثبت شد \n{signature}",
