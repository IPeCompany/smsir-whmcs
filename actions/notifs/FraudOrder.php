...
function smsir_vo_FraudOrder($params){
    $hookname = 'FraudOrder';
...
function smsir_vo_FraudOrder_install($hookname){
    try{
        Capsule::table('smsir_vo_hooks')->insert([
            'name' => $hookname,
            'label' => 'تقلبی شناخته شدن سفارش',
            'params' => vahab::ShowUserDefaultTag('{orderid}','{status}','{invoiceid}','{amount}','{nameservers}','{o_date}'),
            'admin_params' => vahab::ShowUserDefaultTag('{orderid}','{status}','{invoiceid}','{amount}','{nameservers}','{o_date}'),
            'type_send' => 'default',
            'send_for' => 'all',
            'message' => "{firstname} عزیز \nسفارش شماره {orderid} تقلبی شناخته شد \n{signature}",
